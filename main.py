...
class User(db.Model, UserMixin):
    ...
    child: Mapped[List["Comment"]] = relationship()
    id: Mapped[int] = mapped_column(Integer, primary_key=True)
...
def admin_only(function_for_route):
    @wraps(function_for_route)
...
def login():
    ...
    if request.method == "POST":
        ...
        if  not user :
            return render_template('login.html', error="That email doesn't exist try registering", form=login_form)
            ...
            if check_password_hash(pwhash=user.password, password=request.form['password']):
                ...
                return render_template('login.html', error="Incorrect Password", form=login_form)
    ...
    if request.method == "POST":
        if comment_form.validate_on_submit():
            if current_user.is_authenticated:
                ...
                flash('Please Login before you make a comment on Posts.')
                return redirect(url_for('login', error='Please Login before you make a comment on Posts.'))
